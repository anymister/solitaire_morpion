...
class Game:
    ...
    def read_list(self):
        """ Lire 'solitaire.txt' qui contient les cellules du jeu et les ajoutées au tableau rep_cellules  """
        lines = []
        with open('solitaire.txt') as file:  # Chaque ligne contient toutes les valeurs d'une cellule
            lines = file.readlines()
        file.close()
        for i in range(0, len(lines)):
            line = lines[i]
            cel_as_tab = line.split(",")
            cel = Cellule(0, 0, 0, 0, 0, 0, 0)
            ...
            cel.x = int(cel_as_tab[5])
            cel.y = int(cel_as_tab[6])
            self.rep_cellules[cel.y][cel.x] = cel # Mise à jour du tableau de cellules
    ...
    while (cpt_iteration < max_iteration):
        ...
        while not game.crash:  # Tant qu'il reste encore des lignes possibles à jouer
            ...
            if np.array_equal(final_move, [1, 0, 0]):
                ...
                tmp_line = game.rep_playable_lines[random.randint(int((len(game.rep_playable_lines) - 1) / 3),
                                                                  int((len(game.rep_playable_lines) - 1) * 2 / 3))]
            elif np.array_equal(final_move, [0, 0, 1]):
                tmp_line = game.rep_playable_lines[random.randint(int((len(game.rep_playable_lines) - 1) * 2 / 3),
